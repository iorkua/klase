...
class  SurveyAttributionController extends Controller
{
    public function Attributions() 
    {
        $PageTitle = 'Survey Records';
        $PageDescription = '';
        $surveys = DB::connection('sqlsrv')->table('surveyRecord')
            ->where('survey_type', 'Primary Survey')
            ->get(); // Fetch survey records of type Primary Survey

        return view('survey_record.index', compact(
           'PageTitle',
            'PageDescription',
    ...
    public function create()
    {
        ...
        return view('survey_record.create', compact(
            'PageTitle',
            'PageDescription'
        ));
    }
    ...
    public function editSurvey($id)
    {
        ...
        $survey = DB::connection('sqlsrv')->table('surveyRecord')->where('ID', $id)->first(); // Fetch survey record by ID

        if (!$survey) {
            abort(404, 'Survey record not found');
        }

        return view('survey_record.update_survey', compact(
            'PageTitle',
            'PageDescription',
      ...
      public function store(Request $request)
    {
        ...
        return redirect()->route('survey_record.index')->with('success', __('Survey created successfully.'));
    }
    ...
    public function updateSurvey(Request $request)
    {
        ...
        if ($updated) {
            return redirect()->route('survey_record.index')->with('success', 'Survey updated successfully.');
        } else {
    ...
    public function fetchPrimarySurveys(Request $request)
    {
        ...
        $query = DB::connection('sqlsrv')->table('surveyRecord')
    ...
    {
        $survey = DB::connection('sqlsrv')->table('surveyRecord')
            ->where('ID', $id)
            ->first();
    ...
    public function edit($id)
    {
        ...
        $survey = DB::connection('sqlsrv')->table('surveyRecord')->where('ID', $id)->first();

        if (!$survey) {
            abort(404, 'Survey record not found');
        }

        return view('survey_record.edit', compact(
            'PageTitle',
            'PageDescription',
            'survey'
        ));
    ...
    public function viewPlan($id)
    {
        ...
        return view('survey_record.view_plan', compact(
            'PageTitle',
            'PageDescription',
            'survey'
        ));
    ...
    public function destroy($id)
    {
        try {
            ...
            if (!$survey) {
                return redirect()->route('survey_record.index')->with('error', 'Survey record not found.');
            }
            ...
            $deleted = DB::connection('sqlsrv')->table('surveyRecord')->where('ID', $id)->delete();

            if ($deleted) {
                return redirect()->route('survey_record.index')->with('success', 'Survey record deleted successfully.');
            } else {
                return redirect()->route('survey_record.index')->with('error', 'Failed to delete survey record.');
            }
        } catch (\Exception $e) {
            return redirect()->route('survey_record.index')->with('error', 'An error occurred while deleting the survey record.');
        }
    }
}