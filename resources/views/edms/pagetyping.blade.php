                    ...
                    <div class="thumbnails-grid">
                        ...
                        <div class="document-thumbnail {{ $pageData['page_index'] === 0 ? 'active' : '' }}" 
                            ...
                            @if($pageData['type'] === 'pdf_page')
                                <div class="pdf-thumbnail-container" style="width: 100%; height: 100%; position: relative; background: #f8fafc; display: flex; align-items: center; justify-content: center;">
                                    <div class="pdf-thumbnail-loading" style="display: flex; flex-direction: column; align-items: center; justify-content: center; color: #718096;">
                                        <div class="spinner" style="width: 1.5rem; height: 1.5rem; border: 2px solid #e2e8f0; border-top: 2px solid #667eea; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 0.5rem;"></div>
                                        <span style="font-size: 0.75rem;">Loading...</span>
                                    </div>
                                    <img class="pdf-thumbnail-image" style="width: 100%; height: 100%; object-fit: cover; display: none;" alt="PDF Page {{ $pageData['page_number'] }}">
                                    <div class="pdf-thumbnail-fallback" style="display: none; flex-direction: column; align-items: center; justify-content: center; color: #667eea;">
                                        <i data-lucide="file-text" style="width: 2rem; height: 2rem; margin-bottom: 0.5rem;"></i>
                                        <span style="font-size: 0.75rem;">Page {{ $pageData['page_number'] }}</span>
                                    </div>
                                </div>
                            @else
                                ...
                                <i data-lucide="image" class="thumbnail-icon" style="display: none; width: 100%; height: 100%; object-fit: cover; font-size: 2rem; color: #667eea;"></i>
                            @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showPage(index) {
            ...
            if (currentPage.type === 'pdf_page') {
                // For PDF pages, use simple iframe viewer
                const pdfPath = `{{ asset('storage/app/public/') }}/${currentPage.file_path}`;
                documentViewer.innerHTML = `
                    <div style="width: 100%; height: 500px; position: relative;">
                        <iframe 
                            src="${pdfPath}#page=${currentPage.page_number}&zoom=page-fit&toolbar=0&navpanes=0&scrollbar=0"
                            style="width: 100%; height: 100%; border: none; border-radius: 0.5rem; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1);"
                            title="${currentPage.display_name}"
                            onload="this.style.display='block'"
                            onerror="this.style.display='none'; this.nextElementSibling.style.display='block'">
                        </iframe>
                        <div style="display: none; text-align: center; color: #718096; padding: 2rem;">
                            <i data-lucide="file-text" style="width: 4rem; height: 4rem; margin-bottom: 1rem; opacity: 0.5;"></i>
        ...
        // Load all PDF thumbnails using PDF.js
        function loadAllThumbnails() {
            pageData.forEach((page, index) => {
                if (page.type === 'pdf_page') {
                    loadPdfThumbnail(page.file_path, page.page_number, index);
                }
            });
        }
        
        // Load PDF thumbnail using PDF.js
        async function loadPdfThumbnail(filePath, pageNumber, pageIndex) {
            try {
                if (!window.pdfjsLib) {
                    console.error('PDF.js not loaded');
                    showThumbnailFallback(pageIndex);
                    return;
                }
                
                const pdfUrl = `{{ asset('storage/app/public/') }}/${filePath}`;
                ...
                // Calculate scale to fit thumbnail size (150x200 pixels)
                ...
                // Convert canvas to image and update thumbnail
                const imageDataUrl = canvas.toDataURL('image/jpeg', 0.8);
                updateThumbnailImage(pageIndex, imageDataUrl);
        ...
        function updateThumbnailImage(pageIndex, imageDataUrl) {
            const thumbnail = document.querySelector(`[data-page-index="${pageIndex}"]`);
            if (thumbnail) {
                const loadingDiv = thumbnail.querySelector('.pdf-thumbnail-loading');
                const imageElement = thumbnail.querySelector('.pdf-thumbnail-image');
                const fallbackDiv = thumbnail.querySelector('.pdf-thumbnail-fallback');
        ...
        function showThumbnailFallback(pageIndex) {
            const thumbnail = document.querySelector(`[data-page-index="${pageIndex}"]`);
            if (thumbnail) {
                const loadingDiv = thumbnail.querySelector('.pdf-thumbnail-loading');
                const imageElement = thumbnail.querySelector('.pdf-thumbnail-image');
