        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div>
                <button id="batchRegisterBtn" onclick="openBatchRegisterModal()" 
                        class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" 
                        disabled>
                    <i class="fas fa-layer-group"></i> 
                    <span id="batchBtnText">Select Instruments for Batch Registration</span>
                </button>
            </div>
        </div>
...
<script src="{{ asset('js/batch_registration_handler_enhanced.js') }}?v={{ time() }}"></script>
...
function handleMainTableCheckboxChange() {
    ...
    if (checkedCount === 0) {
        ...
        batchBtnText.textContent = 'Select Instruments for Batch Registration';
        batchBtn.classList.add('opacity-50', 'cursor-not-allowed');
    } else if (checkedCount === 1) {
        batchBtn.disabled = true;
        batchBtnText.textContent = `1 Instrument Selected (Need 2+ for Batch)`;
        batchBtn.classList.add('opacity-50', 'cursor-not-allowed');
    } else {
        batchBtn.disabled = false;
        batchBtnText.textContent = `Batch Registration (${checkedCount} Selected)`;
