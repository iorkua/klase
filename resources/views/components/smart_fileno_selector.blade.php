    ...
    <div class="flex justify-start -mt-4 -ml-2">
        <div class="w-1/2">
            ...
            <div id="dropdown-mode" class="fileno-mode">
                <select id="fileno-select" class="w-full p-1.5 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs">
                    <option value="">Select File Number</option>
                    @php
                        $fileNumbers = DB::connection('sqlsrv')
                            ->select("SELECT [id],[kangisFileNo],[mlsfNo],[NewKANGISFileNo] FROM [klas].[dbo].[fileNumber] ORDER BY [id] DESC");
                    @endphp
