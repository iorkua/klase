    ...
    <div class="max-w-6xl mx-auto">
        <div class="bg-white rounded-lg overflow-hidden">
            ...
            <div class="p-6 overflow-y-auto max-h-[80vh]" x-data="{
                ...
                showAll: false,
                userTypeId: '{{ old('user_type', $user->type) }}',
                ...
                async fetchUserLevels(userTypeId) {
                    ...
                    try {
                        ...
                        if (levels.length === 1) {
                            ...
                            // Keep current level if it's valid
                            const currentLevel = levels.find(l => l.name === this.userLevelName);
                            if (currentLevel) {
                                this.userLevelId = currentLevel.id;
                            } else {
                                this.userLevelId = '';
                                this.userLevelName = '';
                            }
                ...
                init() {
                    ...
                    if (this.userTypeName) {
                        // Find the user type ID from the select options
                        const userTypeSelect = document.getElementById('user_type');
                        if (userTypeSelect) {
                            const option = Array.from(userTypeSelect.options).find(opt => opt.text === this.userTypeName);
                            if (option) {
                                this.userTypeId = option.value;
                                this.fetchUserLevels(this.userTypeId);
                            }
                        }
                    }
                }
                                        ...
                                        <div x-show="availableLevels.length > 1 && userTypeId">
                                            <select name="user_level" id="user_level"
                                                ...
                                                <template x-for="level in availableLevels" :key="level.id">
                                                    <option :value="level.id" x-text="level.name" :selected="level.name === userLevelName"></option>
                                                </template>
                                            </select>
                                        </div>
                                    </div>
