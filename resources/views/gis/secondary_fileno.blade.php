...
<div class="bg-gray-50 p-4 rounded-lg mb-4">
    <!-- Main fileno field that gets submitted to the controller -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const filenoField = document.getElementById('fileno'); // Main fileno field
        ...
        unitSelect.addEventListener('change', function() {
            if (!this.value) {
                ...
                // Clear main fileno field
                if (filenoField) filenoField.value = '';
            ...
            // Set the main fileno field to the selected unit's file number
            if (filenoField) filenoField.value = selectedOption.textContent;
        ...
        if (prefixSelect) prefixSelect.addEventListener('change', updateFileNumber);
        
        // Update the file number when serial number changes
        if (serialField) serialField.addEventListener('input', updateFileNumber);
        
        // Set initial file number
        updateFileNumber();
        
        function updateFileNumber() {
            if (!prefixSelect || !yearField || !serialField || !fileNumberPreview) return;
            ...
            if (filenoField) filenoField.value = fullFileNumber; // Set the main fileno field
        }
    });
</script>