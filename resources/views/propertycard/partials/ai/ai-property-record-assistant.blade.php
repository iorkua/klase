...
<div class="container mx-auto py-6 space-y-6 max-w-6xl px-4 sm:px-6 lg:px-8">
  ...
  <div id="extracted-details" class="hidden bg-white rounded-lg shadow border-l-4 border-l-green-500">
    ...
    <div class="p-6 space-y-6">
      @include('propertycard.partials.add_property_record_ai', ['is_ai_assistant' => true])
...
function setupEventListeners() {
  ...
  document.getElementById('file-number-type').addEventListener('change', handleFileNumberTypeChange);
  ...
  for (const pattern of recertificationPatterns.newFileNumber) {
    ...
    if (match?.[1]) {
      ...
      if (fileNo.length > 3) { // Ensure it's not just noise
        ...
        // Determine file number type
        if (fileNo.includes('LKN') || cleanText.toLowerCase().includes('kangis')) {
  ...
  if (!data.fileNo) {
    ...
    for (const pattern of standardFileNoPatterns) {
      ...
      if (match?.[1]) {
        ...
        if (cleanText.toLowerCase().includes('kangis') || fileNo.includes('LKN')) {
          data.fileNumberType = 'kangisFileNo';
          data.kangisFileNo = fileNo;
        } else if (cleanText.toLowerCase().includes('mls') || fileNo.includes('MLS')) {
          data.fileNumberType = 'mlsFileNo';
          data.mlsFileNo = fileNo;
        } else {
          data.fileNumberType = 'kangisFileNo';
...
function populatePropertyForm() {
  ...
  // Ensure inline form root is visible (handled after move as well)
  const dialog = document.getElementById('property-form-dialog');
  if (dialog) {
    // Do nothing in AI inline mode
  }

  // Safely set values into the existing Add New Property form fields if present
  // File numbers from AI can be set into the smart/manual hidden fields
  if (data.kangisFileNo) {
    const kangisHidden = document.getElementById('kangisFileNo');
    if (kangisHidden) kangisHidden.value = data.kangisFileNo;
  }
  if (data.mlsFileNo) {
    const mlsHidden = document.getElementById('mlsFNo');
    if (mlsHidden) mlsHidden.value = data.mlsFileNo;
  }
  if (data.fileNo && !data.kangisFileNo && !data.mlsFileNo) {
    const fileno = document.getElementById('fileno');
    if (fileno) fileno.value = data.fileNo;
  }

  // Plot No and LGA/City
  const plotNoField = document.getElementById('plotNo');
  if (plotNoField && data.plotNo) plotNoField.value = data.plotNo;
  const lgaField = document.getElementById('lga');
  if (lgaField && data.lgsaOrCity) lgaField.value = data.lgsaOrCity;

  // Property Description
  const descField = document.querySelector('#property-record-form textarea#property-description');
  if (descField && data.description) descField.value = data.description;

  // Transaction/instrument mapping if available
  const transType = document.getElementById('transactionType-record');
  if (transType && data.instrument) {
    // Map some common instruments to select options
    const map = {
    ...
    const val = map[data.instrument] || 'Other';
    transType.value = val;
    const event = new Event('change');
    transType.dispatchEvent(event);
  }

  // Parties
  const firstPartyInput = document.querySelector('#property-record-form input[name="Grantor"], #property-record-form input[name="Assignor"], #property-record-form input[name="Mortgagor"], #property-record-form input[name="Lessor"]');
  const secondPartyInput = document.querySelector('#property-record-form input[name="Grantee"], #property-record-form input[name="Assignee"], #property-record-form input[name="Mortgagee"], #property-record-form input[name="Lessee"]');
  if (firstPartyInput && data.assignor) firstPartyInput.value = data.assignor;
  if (secondPartyInput && (data.assignee || data.propertyHolder)) secondPartyInput.value = data.assignee || data.propertyHolder;

  // Registration particulars
  const serialNo = document.getElementById('serialNo');
  const pageNo = document.getElementById('pageNo');
  const volumeNo = document.getElementById('volumeNo');
  if (serialNo && data.serialNo) serialNo.value = data.serialNo;
  if (pageNo && data.page) pageNo.value = data.page;
  if (volumeNo && data.vol) volumeNo.value = data.vol;

  // Trigger Alpine’s auto description rebuild if present
...
function handleFileNumberTypeChange() {
  const fileNumberType = document.getElementById('file-number-type').value;
  const filePrefixField = document.getElementById('file-prefix');
  const fileSerialNoField = document.getElementById('file-serial-no');

  // Set default prefixes based on type
  if (fileNumberType === 'kangisFileNo' && !filePrefixField.value) {
    filePrefixField.placeholder = 'e.g., LKN/RES, ABJ/COM';
  } else if (fileNumberType === 'mlsFileNo' && !filePrefixField.value) {
    filePrefixField.placeholder = 'e.g., MLS/2024';
  }
