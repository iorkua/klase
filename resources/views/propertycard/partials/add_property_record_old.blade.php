...
function propertyRecordForm() {
    return {
        ...
        get description() {
            ...
            // Get street value from either select or custom input
            const streetValue = this.getStreetValue();
            if (streetValue) desc += (desc ? ', ' : '') + streetValue;
            
            // Get district value from either select or custom input
            const districtValue = this.getDistrictValue();
            if (districtValue) desc += (desc ? ', ' : '') + districtValue;
        ...
        // Helper method to get street value
        getStreetValue() {
            const streetSelect = document.getElementById('streetName');
            const streetCustom = document.getElementById('otherStreetName');
            
            if (streetSelect && streetSelect.value === 'other' && streetCustom && streetCustom.value) {
                return streetCustom.value;
            } else if (streetSelect && streetSelect.value && streetSelect.value !== 'other') {
                return streetSelect.value;
            }
            return this.street;
        },
        
        // Helper method to get district value
        getDistrictValue() {
            const districtSelect = document.getElementById('district');
            const districtCustom = document.getElementById('otherDistrict');
            
            if (districtSelect && districtSelect.value === 'other' && districtCustom && districtCustom.value) {
                return districtCustom.value;
            } else if (districtSelect && districtSelect.value && districtSelect.value !== 'other') {
        ...
        init() {
            ...
            // Watch for changes in street and district components
            this.$watch('description', (value) => {
                console.log('📝 Description updated:', value);
            });
            
            // Set up event listeners for the street and district components
            this.setupComponentWatchers();
        },
        
        // Set up watchers for the street and district components
        setupComponentWatchers() {
            // Watch for changes in street select
            const streetSelect = document.getElementById('streetName');
            if (streetSelect) {
                streetSelect.addEventListener('change', () => {
                    this.$nextTick(() => {
                        // Force reactivity update
                        this.street = this.getStreetValue();
                    });
                });
            }
            
            // Watch for changes in street custom input
            const streetCustom = document.getElementById('otherStreetName');
            if (streetCustom) {
                streetCustom.addEventListener('input', () => {
                    this.$nextTick(() => {
                        // Force reactivity update
                        this.street = this.getStreetValue();
                    });
            ...
            // Watch for changes in district select
            const districtSelect = document.getElementById('district');
            if (districtSelect) {
                districtSelect.addEventListener('change', () => {
                    this.$nextTick(() => {
                        // Force reactivity update
                        this.district = this.getDistrictValue();
                    });
                });
            }
            
            // Watch for changes in district custom input
            const districtCustom = document.getElementById('otherDistrict');
            if (districtCustom) {
